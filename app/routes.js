module.exports = function (app, passport, db) {
  ...
  app.get("/profile", isLoggedIn, function (req, res) {
    ...
    db.collection("journalEntry")
      .find({ name: req.user.local.email })
      .toArray((err, result) => {
  ...
  app.post("/dreamJournal", (req, res) => {
    let entry = req.body.journalEntry;
    //let newEntry = entry.replace(/\s/g, "");
    let date = new Date();
    //let newDate = date.replace(/\s/g, "");
    db.collection("journalEntry").insertOne(
      { name: req.user.local.email, entry: entry, date: date },
      (err, result) => {
        if (err) return console.log(err);
